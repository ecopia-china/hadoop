...
class ExfsBlockOutputStream extends OutputStream implements
    StreamCapabilities {

  private static final Logger LOG =
      LoggerFactory.getLogger(ExfsBlockOutputStream.class);
   ...
   * Factory for blocks.
   */
  private final ExfsDataBlocks.BlockFactory blockFactory;
  ...
  private ExfsDataBlocks.DataBlock activeBlock;

  /** Count of blocks uploaded. */
  private long blockCount = 0;

  /** Statistics to build up. */
  private final ExfsInstrumentation.OutputStreamStatistics statistics;
  ...
  ExfsBlockOutputStream(ExfsFileSystem fs,
      ...
      long blockSize,
      ExfsDataBlocks.BlockFactory blockFactory,
      ExfsInstrumentation.OutputStreamStatistics statistics,
    ...
    createBlockIfNeeded();
    LOG.debug("Initialized ExfsBlockOutputStream for {}" +
   ...
   */
  private synchronized ExfsDataBlocks.DataBlock createBlockIfNeeded()
      throws IOException {
    if (activeBlock == null) {
      blockCount++;
   ...
   * @return the active block; null if there isn't one.
   */
  private synchronized ExfsDataBlocks.DataBlock getActiveBlock() {
    return activeBlock;
  }
  ...
  public synchronized void flush() throws IOException {
    ...
    ExfsDataBlocks.DataBlock dataBlock = getActiveBlock();
    if (dataBlock != null) {
      dataBlock.flush();
    }
  }
  ...
  public synchronized void write(byte[] source, int offset, int len)
    ...
    ExfsDataBlocks.validateWriteArgs(source, offset, len);
    checkOpen();
    if (len == 0) {
      return;
    }
    ExfsDataBlocks.DataBlock block = createBlockIfNeeded();
  ...
  public void close() throws IOException {
    ...
    ExfsDataBlocks.DataBlock block = getActiveBlock();
  ...
  private int putObject() throws IOException {
    ...
    final ExfsDataBlocks.DataBlock block = getActiveBlock();
    int size = block.dataSize();
    final ExfsDataBlocks.BlockUploadData uploadData = block.startUpload();
   ...
   */
  ExfsInstrumentation.OutputStreamStatistics getStatistics() {
    return statistics;
  }
  ...
  private class MultiPartUpload {
     ...
     */
    private void uploadBlockAsync(final ExfsDataBlocks.DataBlock block)
        throws IOException {
  ...
  private final class BlockUploadProgress implements ProgressListener {
    private final ExfsDataBlocks.DataBlock block;
     ...
     * into the queue
     */
    private BlockUploadProgress(ExfsDataBlocks.DataBlock block,
